...
def handle_echo(event, cli_config, logger):
    ...
    def sql_fetch(con):
        db_file = 'C:/Users/Andres/AppData/Local/Programs/Python/Python39/Lib/site-packages/pynetdicom/apps/qrscp/instances.sqlite'
        conn = sqlite3.connect(db_file)
        conn.text_factory = sqlite3.OptimizedUnicode
        cursorObj = conn.cursor()

        cursorObj.execute("SELECT * FROM instance")
...
def handle_store(event, storage_dir, db_path, cli_config, logger):
    ...
    # mysql = MySQL()
    miConexion = pymysql.connect( host='localhost', user= 'root', passwd='root', db='dicom' )
    ...
    cur.execute('SELECT * FROM rules WHERE modality = %s',(modality))
    miConexion.commit()
    rules = cur.fetchall()
    ...
    cursor1.execute('SELECT * FROM targets WHERE target = %s',(rules[0][3]))
    miConexion.commit()
    targ = cursor1.fetchall()
    print(targ)
    print(ds)
    print()
    ip = targ[0][2]
    # cmd = 'python -m pynetdicom storescu '+ str(ip)+' 11112 '+ 'C:\Users\Andres\AppData\Local\Programs\Python\Python39\Lib\site-packages\pynetdicom\apps\qrscp\instances'
    print(ip)
    ae = AE()#se cra una instancia AE
    sop = ds.file_meta.MediaStorageSOPClassUID
    tf = ds.file_meta.TransferSyntaxUID
    #tf = ds[0x0002, 0x0010]
    print('El sop es: ')
    print(sop)
    print()
    print(tf)
    ae.add_requested_context(sop,tf)#almacenamiento con explicit little endian
    ...
    if assoc.is_established:
        ...
        status=assoc.send_c_store(ds)#almacenamiento de archivos 
    ...
    # conn = mysql.connect()
    # cursor = conn.cursor()
    # cursor.execute('SELECT * FROM targets WHERE ')
    # conn.commit()
